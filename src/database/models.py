...
class Film(db.Model):
    ...
    actors = db.relationship('Actor', secondary=movies_actors, lazy=True, backref=db.backref(
        'films', lazy=True))  # создали отношение
    ...
    def patching(patcher, object_to_patch):
        ...
        release_date = datetime.strptime(patcher.get(
            'release_date'), '%B %d %Y') if patcher.get('release date') else None
...
class Actor(db.Model):
    ...
    def patching(patcher, object_to_patch):
        ...
        birthday = datetime.strptime(patcher.get(
            'birthday'), '%B %d %Y') if patcher.get('birthday') else None
